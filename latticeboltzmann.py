...
try:
    ...
    print("cPickle import failed! switching to pickle.")
    import pickle
...
#        self.t_0 = 2000.0 # in K
    ...
    def save(self, fname="TLB.dump"):
        data = {"version_id":self.version_id,
                ...
                "t_0":self.t_0,
                "k":self.k,
                "boundry_slip":self.boundry_slip,
                "boundry_stp":self.boundry_stp,
                "stp_state_space":self.stp_state_space,
                "state_space":self.state_space}
        with open(fname, "wb") as fout:
            pickle.dump(data, fout)

    def load(self, fname="TLB.dump"):
        data = None
        with open(fname, "rb") as fin:
            data = pickle.load(fin)
        if self.version_id != data["version_id"]:
            raise IOError("Data dump: " + fname + " version id does not match this software.")
        self.resolution = data["resolution"]
        self.mixing_rate = data["mixing_rate"]
        self.dx = data["dx"]
        self.m = data["m"]
        self.dv = data["dv"]
        self.t_0 = data["t_0"]
        self.k = data["k"]
        self.boundry_slip = data["boundry_slip"]
        self.boundry_stp = data["boundry_stp"]
        self.stp_state_space = data["stp_state_space"]
